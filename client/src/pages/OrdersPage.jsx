      ...
      <div className="relative z-10 mx-auto max-w-5xl px-4 py-16 sm:px-6 lg:px-8">
        ...
        {orders.length === 0 ? (
          <div className="rounded-4xl border border-dashed border-gray-700 bg-gray-900/40 p-16 text-center">
            <h2 className="mb-4 text-2xl font-semibold text-white">No orders found</h2>
            <p className="mb-8 text-gray-400">You haven't placed any orders yet.</p>
            <Link
              to="/"
                  ...
                  <div className="border-t border-gray-800 bg-gray-900/40 p-6 space-y-6">
                    ...
                    <div className="pt-4 border-t border-gray-800 flex justify-between items-center">
                       <div className="text-sm">
                          <p className="text-gray-400">Payment Status: <span className={`font-semibold capitalize ${order.paymentStatus === 'paid' ? 'text-emerald-400' : 'text-amber-400'}`}>{order.paymentStatus}</span></p>
                       </div>
                       <div className="text-right">
                          <p className="text-gray-400 text-sm">Subtotal: ${order.totalAmount.toFixed(2)}</p>
                          <p className="text-xl font-bold text-emerald-400">Total: ${order.totalAmount.toFixed(2)}</p>
                       </div>
                    </div>
                  </div>
                )}
